...
test( "Connexion à une partie existante", async ( { browser } ) =>
{
	...
	await player2Page.getByRole( "button", { name: "Join" } ).first().click();
	await player2Page.getByRole( "button", { name: "Yes" } ).first().click();

	// Vérification du rôle du joueur 1 par le joueur 2.
	await expect( player2Page ).toHaveURL( "/game/selection" );
	await expect( player2Page.getByText( "Player 1" ) ).toBeVisible();

	// Choix du rôle pour le joueur 2.
	await player2Page.getByText( "Choose this role" ).nth( 1 ).check();
...
test( "Pseudonyme déjà utilisé", async ( { browser } ) =>
{
	...
	await expect( player2Page.getByRole( "heading", { name: "Duplicated username" } ) ).toBeVisible();
} );
...
test( "Partie encore active", async ( { page } ) =>
{
	...
	await expect( page.getByRole( "heading", { name: "Data duplication" } ) ).toBeVisible();
} );
